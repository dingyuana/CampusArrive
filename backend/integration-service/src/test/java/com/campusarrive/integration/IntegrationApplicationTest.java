 ...
 * 验证主类可正常实例化，为后续事件链、CDC 同步测试提供基座。</p>
 *
 * <p>注：不使用 @SpringBootTest 以避免在没有 RabbitMQ 连接的环境下加载上下文失败。
 * 上下文加载测试在集成测试阶段（需 RabbitMQ 环境）执行。</p>
 */
@DisplayName("UT-INFRA: 集成中间件服务入口类")
class IntegrationApplicationTest {
    ...
    @Test
    @DisplayName("IntegrationApplication.main 方法存在")
    void mainMethodExists() throws NoSuchMethodException {
        assertNotNull(IntegrationApplication.class.getMethod("main", String[].class),
                "IntegrationApplication 应有 main 方法");
    }
}
